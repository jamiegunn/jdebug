...
var cmdCatalog = []cmdInfo{
	...
	{"W", "workload", "jdebug workload", "the rollout tree + the pod deep-dive, in one scroll",
		"what owns the pod, rollout state, HPA-vs-Deployment fights — THEN limits, probes, exit codes, autoscaling in plain language", "safe · read-only",
		"kubectl; python3", "jdebug topology (tree only) or jdebug why (pod layer only); y is an alias"},
